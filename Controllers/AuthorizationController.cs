...
public class AuthorizationController : Controller
{
    ...
    public async Task<IActionResult> Authorize()
    {
            ...
            DateTimeOffset.UtcNow - result.Properties.IssuedUtc > TimeSpan.FromSeconds(request.MaxAge.Value)))
        {
            ...
            return Challenge(
                ...
                {
                    //challenge的loginpath + ?returnUrl=xxx
        ...
        var authorizations = await _authorizationManager.FindAsync(
            subject: user.Id.ToString(),// await _userManager.GetUserIdAsync(user),
        ...
        switch (await _applicationManager.GetConsentTypeAsync(application))
        {
            ...
            case ConsentTypes.Explicit when authorizations.Any() && !request.HasPrompt(Prompts.Consent):
                ...
                identity.SetClaim(Claims.Subject, user.Id.ToString())
                        ...
                        .SetClaim("__tenant__", user.Tenant);
    ...
    public async Task<IActionResult> Exchange()
    {
        ...
        {
            var tenantId = (string?)request.GetParameter("tenant");
            if (tenantId == null)
            {
            ...
            var ti = await _tenantStore.TryGetByIdentifierAsync(tenantId);
            if (ti == null)
            {
                var properties = new AuthenticationProperties(new Dictionary<string, string>
                {
            ...
            var checkResult = await _signInService.PasswordSignInAsync(ti, request.Username, request.Password);
            if (!checkResult.Item1)
            {
                var properties = new AuthenticationProperties(new Dictionary<string, string>
                {
                    [OpenIddictServerAspNetCoreConstants.Properties.Error] = Errors.InvalidGrant,
                    [OpenIddictServerAspNetCoreConstants.Properties.ErrorDescription] =
                        "The tenant/username/password couple is invalid."
                });
                return Forbid(properties, OpenIddictServerAspNetCoreDefaults.AuthenticationScheme);
            }
            var user = checkResult.Item2;
            ...
            identity.SetClaim(Claims.Subject, user.Id.ToString())
                    .SetClaim(Claims.Email, user.Email)
                    .SetClaim("__tenant__", tenantId);// 设置租户信息到clamins中
                    //.SetClaims(Claims.Role, (await _userManager.GetRolesAsync(user)).ToImmutableArray());
        ...
        if (request.IsAuthorizationCodeGrantType() || request.IsRefreshTokenGrantType())
        {
            ...
            var user = await _userManager.FindByIdAsync(result.Principal.GetClaim(Claims.Subject), result.Principal.GetClaim("__tenant__"));
            if (user is null)
            {
                return Forbid(
                    authenticationSchemes: OpenIddictServerAspNetCoreDefaults.AuthenticationScheme,
